...
from util import hinge_loss, byol_loss, L2_loss
...
class Trainer():
    def __init__(self, config):
        ...
        self.lambda_cr = config.lambda_cr
    ...
    def train(self):
        ...
        while(True):
            for iters, (img, img_aug_1, img_aug_2) in enumerate(self.dloader):
                ...
                # D loss (2) - CR loss
                if self.config.cr_type == 'logit':
                    D_real_score_cr = self.D(img_aug_1)
                    D_cr_loss = L2_loss(D_real_score, D_real_score_cr)
                elif self.config.cr_type == 'feature':
                    D_feature = self.D.extract_feature(img)
                    D_feature_aug = self.D.extract_feature(img_aug_1)
                    D_cr_loss = L2_loss(D_feature, D_feature_aug)
                
                # D loss - Adversarial loss + BYOL loss
                D_loss = (self.lambda_adv * D_adv_loss) + (self.lambda_cr * D_cr_loss)
                ...
                print('  ** [Epoch : %d / Total iter : %d] => D adv loss : %f / D cr loss : %f / G adv loss : %f / FM loss : %f / Queue util : %f'% (epochs+1, total_iters, D_adv_loss.item(), D_cr_loss.item(), G_adv_loss.item(), feature_matching_loss.item(), torch.mean(self.queue['used'].float())))

                # Report losses
                if (total_iters - 1) % self.config.report_loss_iter == 0 and self.config.use_nsml:
                    log = OrderedDict()
                    log['loss/D_adv'] = D_adv_loss.item()
                    log['loss/D_cr'] = D_cr_loss.item()
